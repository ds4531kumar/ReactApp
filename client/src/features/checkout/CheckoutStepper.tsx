...
import { Address } from "../../app/model/user";
import {
    ConfirmationToken,
...
const steps = ["Address", "Pay", "Review"];
export default function CheckoutStepper() {
    ...
    const handleNext = async () => {
        if (activeStep === 0 && saveAddressChecked && elements) {
            const address = await getStripAddress();
        ...
        if (activeStep === 1) {
            ...
            const stripResult = await stripe.createConfirmationToken({
                elements,
            });
            if (stripResult.error)
                return toast.error(stripResult.error.message);
    ...
    const createOrderModel = async () => {
        const shippingAddress = await getStripAddress();
